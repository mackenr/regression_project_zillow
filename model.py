...
def linearmodel(X_train, X_validate, y_train, y_validate, degree=1):
    try:
        ...
        rmse_train = mean_squared_error(
            y_train.iloc[:, 0], y_train.taxvaluedollarcnt_pred_lm
        ) ** (1 / 2)
        ...
        rmse_validate = mean_squared_error(
            y_validate.iloc[:, 0], y_validate.taxvaluedollarcnt_pred_lm
        ) ** (1 / 2)
...
def full_model(X_train, X_validate, y_train, y_validate, X_test, y_test, best):
    ...
    try:
        ...
        rmse_train = mean_squared_error(
            y_train.iloc[:, 0], y_train.taxvaluedollarcnt_pred
        ) ** (1 / 2)
        # predict validate
        y_validate["taxvaluedollarcnt_pred"] = model.predict(X_validate)
        rmse_validate = mean_squared_error(
            y_validate.iloc[:, 0], y_validate.taxvaluedollarcnt_pred
        ) ** (1 / 2)
        # predict test
        y_test["taxvaluedollarcnt_pred"] = model.predict(X_test)
        rmse_test = mean_squared_error(
            y_test.iloc[:, 0], y_test.taxvaluedollarcnt_pred
        ) ** (1 / 2)
        ...
        plt.scatter(
            y_train.iloc[:, 0],
            y_train.taxvaluedollarcnt_pred,
            alpha=0.5,
            color="yellow",
            s=100,
            label=f"Model: {model_name} | Train",
        )
        plt.scatter(
            y_validate.iloc[:, 0],
            y_validate.taxvaluedollarcnt_pred,
            alpha=0.5,
            color="green",
            s=100,
            label=f"Model: {model_name} | Validate",
        )
        plt.scatter(
            y_test.iloc[:, 0],
            y_test.taxvaluedollarcnt_pred,
            alpha=0.5,
            color="red",
            s=100,
        ...
        plt.savefig(
            fname=f"TrainValTestGraph{model_name}.png", dpi="figure", format="png"
        )
        plt.show()
        return rmseDF
    except:
...
def nonlinearmodel(
    ...
    y_train,
    y_validate,
    model=TweedieRegressor(power=1.5, alpha=0.25),
):
    try:
        # create the model object
        ...
        # evaluate: rmse
        rmse_train = mean_squared_error(
            y_train.iloc[:, 0], y_train.taxvaluedollarcnt_pred_glm
        ) ** (1 / 2)
        # predict validate
        y_validate["taxvaluedollarcnt_pred_glm"] = model.predict(X_validate)
        # evaluate: rmse
        rmse_validate = mean_squared_error(
            y_validate.iloc[:, 0], y_validate.taxvaluedollarcnt_pred_glm
        ) ** (1 / 2)
...
def baseline_rmse(y_train, y_validate):
    ...
    rmse_train = mean_squared_error(
        y_train.iloc[:, 0], y_train.taxvaluedollarcnt_pred_mean
    ) ** (1 / 2)
    rmse_validate = mean_squared_error(
        y_validate.iloc[:, 0], y_validate.taxvaluedollarcnt_pred_mean
    ) ** (1 / 2)
    model_name = "Mean"
    ...
    rmse_train = mean_squared_error(
        y_train.iloc[:, 0], y_train.taxvaluedollarcnt_pred_median
    ) ** (1 / 2)
    rmse_validate = mean_squared_error(
        y_validate.iloc[:, 0], y_validate.taxvaluedollarcnt_pred_median
    ) ** (1 / 2)
    model_name = "Median"
    degree = model_params = model_obj = "N/A"
    model_tupple = (
    ...
    rmseDF = pd.DataFrame(
        model_list,
        columns=["Model[RSME]", "Params", "Degree", "Train", "Validate", "model_obj"],
    )
...
def regmodelbest(X_train, y_train, X_validate, y_validate, X_test, y_test, random=123):
    ...
    for deg in range(1, 5):
        try:
            model_tupple = linearmodel(
                X_train, X_validate, y_train, y_validate, degree=deg
            )
            model_list.append(model_tupple)
        except:
            print("error")
    ...
    try:
        model_tupple = nonlinearmodel(
            X_train, X_validate, y_train, y_validate, model=ElasticNet(tol=1e-6)
        )
        model_list.append(model_tupple)
    except:
        print("error")
    ...
    try:
        model_tupple = nonlinearmodel(
            X_train, X_validate, y_train, y_validate, model=LassoLarsIC()
        )
        model_list.append(model_tupple)
    except:
        print("error")
    ...
    try:
        model_tupple = nonlinearmodel(
            X_train, X_validate, y_train, y_validate, model=ARDRegression()
        )
        model_list.append(model_tupple)
    except:
        print("error")
    ...
    try:
        model_tupple = nonlinearmodel(
            X_train, X_validate, y_train, y_validate, model=LassoLars(alpha=1.0)
        )
        model_list.append(model_tupple)
    except:
        print("error")
    ...
    try:
        model_tupple = nonlinearmodel(
            ...
            y_train,
            y_validate,
            model=BayesianRidge(n_iter=int(1e4), tol=1e-8),
        )
        model_list.append(model_tupple)
    except:
        print("error")
    ...
    rmseDF_full = bestTest(
        X_train, y_train, X_validate, y_validate, X_test, y_test, best
    )
...
def single_split_many_return(df, totarget="taxvaluedollarcnt"):
    ...
    X_train, y_train, X_validate, y_validate, X_test, y_test = bigX_little_y(
        train, validate, test, target=totarget
    )
    fulllist = []
    ...
    (
        ...
        X_la_test,
        y_la_test,
    ) = bigX_little_y(la_train, la_validate, la_test, target=totarget)
    lalist = []
    ...
    (
        ...
        X_orange_test,
        y_orange_test,
    ) = bigX_little_y(orange_train, orange_validate, orange_test, target=totarget)
    oclist = []
    ...
    (
        ...
        X_ventura_test,
        y_ventura_test,
    ) = bigX_little_y(ventura_train, ventura_validate, ventura_test, target=totarget)
    venturalist = []
    ...
    (
        X_la_train_scaled,
        y_la_train_scaled,
        X_la_validate_scaled,
        y_la_validate_scaled,
        X_la_test_scaled,
        y_la_test_scaled,
    ) = bigX_little_y(
        la_train_scaled, la_validate_scaled, la_test_scaled, target=totarget
    )
    ...
    (
        X_orange_train_scaled,
        y_orange_train_scaled,
        X_orange_validate_scaled,
        y_orange_validate_scaled,
        X_orange_test_scaled,
        y_orange_test_scaled,
    ) = bigX_little_y(
        orange_train_scaled, orange_validate_scaled, orange_test_scaled, target=totarget
    )
    ...
    (
        X_ventura_train_scaled,
        y_ventura_train_scaled,
        X_ventura_validate_scaled,
        y_ventura_validate_scaled,
        X_ventura_test_scaled,
        y_ventura_test_scaled,
    ) = bigX_little_y(
        ventura_train_scaled,
        ventura_validate_scaled,
        ventura_test_scaled,
        target=totarget,
    )
    ...
    (
        ...
        X_scale_test,
        y_scale_test,
    ) = bigX_little_y(train_scaled, validate_scaled, test_scaled, target=totarget)
